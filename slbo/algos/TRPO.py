...
def conj_grad(
    mat_mul_vec: Callable[[np.ndarray], np.ndarray],
    b,
    n_iters=10,
    residual_tol=1e-10,
    verbose=False,
...
class TRPO(nn.Module):
    def __init__(
        ...
        with self.scope:
            ...
            self.op_feed_params = tf.placeholder(
                dtype=tf.float32, shape=[None], name="feed_params"
            )

            self.op_tangents = tf.placeholder(
                dtype=tf.float32,
                shape=[nn.utils.n_parameters(self.policy.parameters())],
            )
            self.op_ent_coef = tf.placeholder(
                dtype=tf.float32, shape=[], name="ent_coef"
            )
        ...
        (
            self.op_sync_old,
            self.op_hessian_vec_prod,
            self.op_flat_grad,
        ) = self.compute_natural_grad(self.op_loss, self.op_mean_kl, self.op_tangents)

        self.op_vf_loss, self.op_train_vf, self.op_vf_grad_norm = self.compute_vf(
            self.op_states, self.op_returns
        )
    ...
    def forward(self, states, actions, advantages, ent_coef):
        ...
        return (
            mean_kl,
            loss,
            distribution.stddev().log().reduce_mean().exp(),
            distribution.mean().norm(axis=1).reduce_mean() / np.sqrt(10),
            policy_loss,
        )
    ...
    def compute_vf(self, states, returns, max_grad_norm=2.0):
        ...
        grads_and_vars = optimizer.compute_gradients(vf_loss, var_list=vf_params)
        clip_grads, op_vf_grad_norm = tf.clip_by_global_norm(
        ...
        train_vf = optimizer.apply_gradients(clip_grads_and_vars)

        return vf_loss, train_vf, op_vf_grad_norm

    @nn.make_method()
    def get_vf_loss(self, states, returns) -> List[np.ndarray]:
        pass
    ...
    def get_hessian_vec_prod(self, states, tangents, actions) -> List[np.ndarray]:
        pass

    @nn.make_method(fetch="loss")
    def get_loss(self, states, actions, advantages, ent_coef) -> List[np.ndarray]:
        pass

    def train(self, ent_coef, samples, advantages, values, skip_policy_update=False):
        ...
        if skip_policy_update:
            ...
            vf_loss, vf_grad_norm = self.train_vf(vf_dataset, skip_update=False)

            return 0.0, 0.0, vf_loss, vf_grad_norm
        ...
        def fisher_vec_prod(x):
            return (
                self.get_hessian_vec_prod(samples.state, x, samples.action)
                + self.cg_damping * x
            )

        assert np.isfinite(grad).all()
        nat_grad = conj_grad(
            fisher_vec_prod, grad, n_iters=self.n_cg_iters, verbose=False
        )
        ...
        vf_loss, vf_grad_norm = self.train_vf(vf_dataset)

        return dist_mean, dist_std, vf_loss, vf_grad_norm

    def train_vf(self, dataset: Dataset, skip_update=False):
        if not skip_update:
            for _ in range(self.n_vf_iters):
                for subset in dataset.iterator(64):
                    self.get_vf_loss(
                        subset.state, subset.return_, fetch="train_vf vf_loss"
                    )
            for param in self.parameters():
                param.invalidate()
        vf_loss, vf_grad_norm = self.get_vf_loss(
            dataset.state, dataset.return_, fetch="vf_loss vf_grad_norm"
        )
        return vf_loss, vf_grad_norm
