...
class NeuralNetEnv:
    def __init__(self, env, inner_env, dynamics):
        ...
        self.is_done = getattr(
            inner_env, "is_done", lambda x, y: np.asarray([False] * len(x))
        )
    ...
    def step(self, action, use_states=None):
        ...
        if use_states is not None:
            ...
            next_observation[:obs_dim] = np.clip(
                next_observation[:obs_dim], *self.observation_space.bounds
            )
        ...
        if hasattr(self.inner_env, "env"):
            reward = -self.inner_env.env.cost_np_vec(
                self.state[None], action[None], np.array([next_observation])
            )[0]
        else:
            reward = -self.inner_env.cost_np_vec(
                self.state[None], action[None], np.array([next_observation])
            )[0]

        done = self.is_done(self.state[None], next_observation)[0]
        self.state = np.reshape(next_observation, -1)
        return self.inner_env.step(
...
class VecSimpleEnv(object):
    ...
    def step(self, actions, use_states=None):
        ...
        if use_states is not None:
            obs_dim = self.env.observation_space.shape[0]
            next_observations[:, :obs_dim] = np.clip(
                next_observations[:, :obs_dim], *self.env.observation_space.bounds
            )
            next_observations[:, :obs_dim] = np.clip(
                next_observations[:, :obs_dim], -1e5, 1e5
            )
        else:
            next_observations = np.clip(
                next_observations, *self.env.observation_space.bounds
            )
            next_observations = np.clip(next_observations, -1e5, 1e5)
        if hasattr(self.env.inner_env, "cost_np_vec"):
            rewards = -self.env.inner_env.cost_np_vec(
                self.states, actions, next_observations
            )
        else:
            rewards = -self.env.inner_env.env.cost_np_vec(
                self.states, actions, next_observations
            )
